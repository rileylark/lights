...
Elm.CycleApp = Elm.CycleApp || {};
Elm.CycleApp.make = function (_elm) {
   "use strict";
   _elm.CycleApp = _elm.CycleApp || {};
   if (_elm.CycleApp.values) return _elm.CycleApp.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var create = function (config) {
      ...
      var notify = function (notification) {    return A2($Signal.send,actionsMailbox.address,$Maybe.Just(notification));};
      var model = A3($Signal.foldp,update,config.initialState,actionsMailbox.signal);
      var notificationAddress = A2($Signal.forwardTo,actionsMailbox.address,$Maybe.Just);
      var spawn = function (task) {
         return A2($Task.mapError,
         function (_p1) {
            return {ctor: "_Tuple0"};
         },
         A2($Task.andThen,$Task.spawn(task),function (threadId) {    return $Task.succeed({ctor: "_Tuple0"});}));
      };
      var instructionMailbox = $Signal.mailbox(spawn($Task.succeed({ctor: "_Tuple0"})));
      var taskAddress = A2($Signal.forwardTo,instructionMailbox.address,spawn);
      return {output: A2($Signal.map,A2(config.view,notificationAddress,taskAddress),model)
             ,tasksToRun: instructionMailbox.signal
             ,notificationAddress: notificationAddress};
   };
   var App = F3(function (a,b,c) {    return {output: a,tasksToRun: b,notificationAddress: c};});
   var Config = F3(function (a,b,c) {    return {initialState: a,update: b,view: c};});
   return _elm.CycleApp.values = {_op: _op,create: create,App: App,Config: Config};
};
Elm.GameMath = Elm.GameMath || {};
Elm.GameMath.make = function (_elm) {
   "use strict";
   _elm.GameMath = _elm.GameMath || {};
   if (_elm.GameMath.values) return _elm.GameMath.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var sortClockwise = F2(function (_p0,pointsToSort) {
      var _p1 = _p0;
      var angle = function (_p2) {    var _p3 = _p2;return -1 * A2($Basics.atan2,_p3._1 - _p1._1,_p3._0 - _p1._0);};
      var addAngle = function (point) {    return {angle: angle(point),point: point};};
      var pointsWithAngles = A2($List.map,addAngle,pointsToSort);
      var sorted = A2($List.sortBy,function (_) {    return _.angle;},pointsWithAngles);
      return A2($List.map,function (_) {    return _.point;},sorted);
   });
   var findClosest = F2(function (_p4,targets) {
      var _p5 = _p4;
      var distances = A2($List.map,
      function (_p6) {
         var _p7 = _p6;
         var _p9 = _p7._1;
         var _p8 = _p7._0;
         var distance = Math.pow(_p8 - _p5._0,2) + Math.pow(_p9 - _p5._1,2);
         return {ctor: "_Tuple2",_0: distance,_1: {ctor: "_Tuple2",_0: _p8,_1: _p9}};
      },
      targets);
      var maybeMinimum = $List.minimum(distances);
      var _p10 = maybeMinimum;
      if (_p10.ctor === "Just") {
            return $Maybe.Just(_p10._0._1);
         } else {
            return $Maybe.Nothing;
         }
   });
   var makeSegments = function (points) {
      var maybeTail = $List.tail(points);
      var maybeHead = $List.head(points);
      var _p11 = maybeTail;
      if (_p11.ctor === "Just") {
            var _p12 = maybeHead;
            if (_p12.ctor === "Just") {
                  ...
                  points,
                  A2($Basics._op["++"],_p11._0,_U.list([_p12._0])));
               } else {
                  return _U.list([]);
               }
         } else {
   ...
   var subtract = F2(function (_p14,_p13) {    var _p15 = _p14;var _p16 = _p13;return {ctor: "_Tuple2",_0: _p15._0 - _p16._0,_1: _p15._1 - _p16._1};});
   var dot2d = F2(function (_p18,_p17) {    var _p19 = _p18;var _p20 = _p17;return _p19._0 * _p20._0 + _p19._1 * _p20._1;});
   var cross2d = F2(function (_p22,_p21) {    var _p23 = _p22;var _p24 = _p21;return _p23._0 * _p24._1 - _p23._1 * _p24._0;});
   var intersectParams = F2(function (_p26,_p25) {
      var _p27 = _p26;
      var _p32 = _p27._0;
      var _p28 = _p25;
      var _p31 = _p28._1;
      var _p30 = _p28._0;
      var bDirection = A2(subtract,_p31,_p30);
      var aDirection = A2(subtract,_p27._1,_p32);
      if (_U.eq(A2(cross2d,aDirection,bDirection),0)) return $Maybe.Nothing; else {
            var v2 = A2(subtract,_p31,_p30);
            var v1 = A2(subtract,_p32,_p30);
            var _p29 = aDirection;
            var aDx = _p29._0;
            var aDy = _p29._1;
            var v3 = {ctor: "_Tuple2",_0: 0 - aDy,_1: aDx};
            var t = A2(cross2d,v2,v1) / A2(dot2d,v2,v3);
            var u = A2(dot2d,v1,v3) / A2(dot2d,v2,v3);
            return $Maybe.Just({ctor: "_Tuple2",_0: t,_1: u});
         }
   });
   var intersectSegment = F2(function (segA,segB) {
      var maybeIntersectionParams = A2(intersectParams,segA,segB);
      var _p33 = maybeIntersectionParams;
      if (_p33.ctor === "Nothing") {
            return false;
         } else {
            var _p35 = _p33._0._1;
            var _p34 = _p33._0._0;
            return _U.cmp(_p34,0) > -1 && (_U.cmp(_p34,1) < 1 && (_U.cmp(_p35,0) > -1 && _U.cmp(_p35,1) < 1));
         }
   });
   var visible = F3(function (targetPoint,fromPoint,potentialObstacles) {
      var criticalSegment = {ctor: "_Tuple2",_0: fromPoint,_1: targetPoint};
      return $Basics.not(A2($List.any,intersectSegment(criticalSegment),potentialObstacles));
   });
   var intersect = F2(function (ray,segment) {
      var maybeIntersectionParams = A2(intersectParams,{ctor: "_Tuple2",_0: ray.startAt,_1: ray.goTowards},segment);
      var _p36 = maybeIntersectionParams;
      if (_p36.ctor === "Nothing") {
            return $Maybe.Nothing;
         } else {
            var _p40 = _p36._0._1;
            var _p39 = _p36._0._0;
            if (_U.cmp(_p39,0) > -1 && (_U.cmp(0,_p40) < 1 && _U.cmp(_p40,1) < 1)) {
                  var _p37 = A2(subtract,ray.goTowards,ray.startAt);
                  var rayDx = _p37._0;
                  var rayDy = _p37._1;
                  var _p38 = ray.startAt;
                  var rayStartX = _p38._0;
                  var rayStartY = _p38._1;
                  return $Maybe.Just({ctor: "_Tuple2",_0: rayStartX + _p39 * rayDx,_1: rayStartY + _p39 * rayDy});
               } else return $Maybe.Nothing;
         }
   });
   var cast = F3(function (fromPoint,againstSegments,throughPoint) {
      var ray = {startAt: fromPoint,goTowards: throughPoint};
      var maybeIntersections = A2($List.map,intersect(ray),againstSegments);
      var defaulted = A2($List.map,$Maybe.withDefault({ctor: "_Tuple2",_0: 100000,_1: 100000}),maybeIntersections);
      return A2(findClosest,fromPoint,defaulted);
   });
   var castSpray = F3(function (fromPoint,againstSegments,throughPoint) {
      var _p41 = throughPoint;
      var x = _p41._0;
      var y = _p41._1;
      var right = {ctor: "_Tuple2",_0: x + 1.0e-4,_1: y + 1.0e-4};
      var left = {ctor: "_Tuple2",_0: x - 1.0e-4,_1: y - 1.0e-4};
      return A2($List.filterMap,A2(cast,fromPoint,againstSegments),_U.list([right,left]));
   });
   var Ray = F2(function (a,b) {    return {startAt: a,goTowards: b};});
   return _elm.GameMath.values = {_op: _op
                                 ,Ray: Ray
                                 ,cross2d: cross2d
                                 ,dot2d: dot2d
                                 ,subtract: subtract
                                 ...
                                 ,intersect: intersect
                                 ,sortClockwise: sortClockwise};
};
Elm.Scene = Elm.Scene || {};
Elm.Scene.make = function (_elm) {
   "use strict";
   _elm.Scene = _elm.Scene || {};
   if (_elm.Scene.values) return _elm.Scene.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $GameMath = Elm.GameMath.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var spiderWeb = F2(function (centerPoint,listOfShapes) {
      var collisionSegments = $List.concat(A2($List.map,$GameMath.makeSegments,listOfShapes));
      var getIntersectionsForShape = function (shape) {    return $List.concat(A2($List.map,A2($GameMath.castSpray,centerPoint,collisionSegments),shape));};
      var rayIntersections = $List.concat(A2($List.map,getIntersectionsForShape,listOfShapes));
      var sorted = A2($GameMath.sortClockwise,centerPoint,rayIntersections);
      return sorted;
   });
   var fuzzyLights = F2(function (centerPoint,listOfShapes) {
      var _p0 = centerPoint;
      var x = _p0._0;
      var y = _p0._1;
      var pointSources = _U.list([{ctor: "_Tuple2",_0: x + 3,_1: y + 3}
                                 ,{ctor: "_Tuple2",_0: x + 3,_1: y - 3}
                                 ,{ctor: "_Tuple2",_0: x - 3,_1: y + 3}
                                 ,{ctor: "_Tuple2",_0: x - 3,_1: y - 3}]);
      return A2($List.map,function (pointSource) {    return A2(spiderWeb,pointSource,listOfShapes);},pointSources);
   });
   var randomPoint = F2(function (lower,upper) {    return A2($Random.pair,A2($Random.$float,lower,upper),A2($Random.$float,lower,upper));});
   var makePoint = F3(function (seed,lower,upper) {    return A2($Random.generate,A2(randomPoint,lower,upper),seed);});
   var randomShape = F3(function (seed,lower,upper) {
      var addPoint = F2(function (_p2,_p1) {
         var _p3 = _p1;
         var _p4 = A3(makePoint,_p3._1,lower,upper);
         var nextPoint = _p4._0;
         var nextSeed = _p4._1;
         return {ctor: "_Tuple2",_0: A2($List._op["::"],nextPoint,_p3._0),_1: nextSeed};
      });
      return A3($List.foldr,addPoint,{ctor: "_Tuple2",_0: _U.list([]),_1: seed},_U.range(1,3));
   });
   var initialSeed = $Random.initialSeed(124);
   var randomTriangles = F2(function (lower,upper) {
      var numTriangles = 5;
      var range = upper - lower;
      var addShape = F2(function (index,_p5) {
         var _p6 = _p5;
         var max = lower + range * (index / numTriangles);
         var min = lower + range * ((index - 1) / numTriangles);
         var _p7 = A3(randomShape,_p6._1,min,max);
         var nextShape = _p7._0;
         var nextSeed = _p7._1;
         return {ctor: "_Tuple2",_0: A2($List._op["::"],nextShape,_p6._0),_1: nextSeed};
      });
      var _p8 = A3($List.foldr,addShape,{ctor: "_Tuple2",_0: _U.list([]),_1: initialSeed},_U.range(1,5));
      var shapes = _p8._0;
      var nextSeed = _p8._1;
      return shapes;
   });
   return _elm.Scene.values = {_op: _op
                              ,initialSeed: initialSeed
                              ,randomPoint: randomPoint
                              ,makePoint: makePoint
                              ,randomShape: randomShape
                              ,randomTriangles: randomTriangles
                              ,fuzzyLights: fuzzyLights
                              ,spiderWeb: spiderWeb};
};
Elm.Levels = Elm.Levels || {};
Elm.Levels.Common = Elm.Levels.Common || {};
Elm.Levels.Common.make = function (_elm) {
   "use strict";
   _elm.Levels = _elm.Levels || {};
   _elm.Levels.Common = _elm.Levels.Common || {};
   if (_elm.Levels.Common.values) return _elm.Levels.Common.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $GameMath = Elm.GameMath.make(_elm),
   ...
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Scene = Elm.Scene.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var calculateLightMaps = function (levelState) {
      return A2($Scene.fuzzyLights,levelState.lightPosition,A2($List._op["::"],levelState.border,levelState.shapes));
   };
   var calculateDetectors = function (levelState) {
      var obstacleSegments = $List.concat(A2($List.map,$GameMath.makeSegments,levelState.shapes));
      var calcDetector = function (detector) {
         var visible = A3($GameMath.visible,detector.position,levelState.lightPosition,obstacleSegments);
         var numVisible = visible ? 1 : 0;
         return {ctor: "_Tuple2",_0: detector,_1: numVisible};
      };
      return A2($List.map,calcDetector,levelState.detectors);
   };
   var calculateLevel = function (levelState) {    return {lightMaps: calculateLightMaps(levelState),detectors: calculateDetectors(levelState)};};
   var update = F2(function (action,oldState) {
      var intermediateState = function () {    var _p0 = action;return _U.update(oldState,{lightPosition: _p0._0});}();
      return {ctor: "_Tuple2",_0: intermediateState,_1: calculateLevel(intermediateState)};
   });
   var MoveLight = function (a) {    return {ctor: "MoveLight",_0: a};};
   var LevelState = F4(function (a,b,c,d) {    return {lightPosition: a,shapes: b,border: c,detectors: d};});
   var Detector = F2(function (a,b) {    return {position: a,litGoal: b};});
   return _elm.Levels.Common.values = {_op: _op
                                      ,Detector: Detector
                                      ,LevelState: LevelState
                                      ,MoveLight: MoveLight
                                      ,update: update
                                      ,calculateLevel: calculateLevel
                                      ,calculateDetectors: calculateDetectors
                                      ,calculateLightMaps: calculateLightMaps};
};
Elm.Levels = Elm.Levels || {};
Elm.Levels.Level01 = Elm.Levels.Level01 || {};
Elm.Levels.Level01.make = function (_elm) {
   "use strict";
   _elm.Levels = _elm.Levels || {};
   _elm.Levels.Level01 = _elm.Levels.Level01 || {};
   if (_elm.Levels.Level01.values) return _elm.Levels.Level01.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Levels$Common = Elm.Levels.Common.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Scene = Elm.Scene.make(_elm),
   ...
   var initialState = function () {
      var detector = F2(function (_p0,goal) {    var _p1 = _p0;return {position: {ctor: "_Tuple2",_0: _p1._0,_1: _p1._1},litGoal: goal};});
      var lit = function (position) {    return A2(detector,position,1);};
      var shaded = function (position) {    return A2(detector,position,0);};
      return {lightPosition: {ctor: "_Tuple2",_0: 150,_1: 0}
             ,shapes: A2($Scene.randomTriangles,-200,200)
             ,border: _U.list([{ctor: "_Tuple2",_0: -200,_1: -200}
                              ,{ctor: "_Tuple2",_0: 200,_1: -200}
                              ,{ctor: "_Tuple2",_0: 200,_1: 200}
                              ,{ctor: "_Tuple2",_0: -200,_1: 200}])
             ,detectors: _U.list([lit({ctor: "_Tuple2",_0: 50,_1: 50})
                                 ,shaded({ctor: "_Tuple2",_0: -50,_1: -50})
                                 ,lit({ctor: "_Tuple2",_0: -50,_1: 50})
                                 ,shaded({ctor: "_Tuple2",_0: 100,_1: -40})])};
   }();
   return _elm.Levels.Level01.values = {_op: _op,initialState: initialState};
};
Elm.State = Elm.State || {};
Elm.State.make = function (_elm) {
   ...
   $Debug = Elm.Debug.make(_elm),
   $Levels$Common = Elm.Levels.Common.make(_elm),
   $Levels$Level01 = Elm.Levels.Level01.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var convertMousePosition = F2(function (_p1,_p0) {
      var _p2 = _p1;
      var _p3 = _p0;
      var maxY = $Basics.toFloat(_p2._1) / 2;
      var cursorY = ($Basics.toFloat(_p3._1) - maxY) * -1;
      var maxX = $Basics.toFloat(_p2._0) / 2;
      var cursorX = $Basics.toFloat(_p3._0) - maxX;
      return {ctor: "_Tuple2",_0: cursorX,_1: cursorY};
   });
   var update = F2(function (action,state) {
      var _p4 = action;
      if (_p4.ctor === "MouseMoved") {
            var _p6 = _p4._0;
            var _p5 = state.gameState;
            var level = _p5._0;
            var baked = _p5._1;
            var cursorPosition = A2(convertMousePosition,state.windowDimensions,_p6);
            return _U.update(state,{mousePosition: _p6,gameState: A2($Levels$Common.update,$Levels$Common.MoveLight(cursorPosition),level)});
         } else {
            return _U.update(state,{windowDimensions: _p4._0});
         }
   });
   var initialState = {windowDimensions: {ctor: "_Tuple2",_0: 600,_1: 600}
                      ,mousePosition: {ctor: "_Tuple2",_0: 50,_1: 200}
                      ,gameState: {ctor: "_Tuple2",_0: $Levels$Level01.initialState,_1: $Levels$Common.calculateLevel($Levels$Level01.initialState)}};
   var Model = F3(function (a,b,c) {    return {windowDimensions: a,mousePosition: b,gameState: c};});
   var WindowSized = function (a) {    return {ctor: "WindowSized",_0: a};};
   var MouseMoved = function (a) {    return {ctor: "MouseMoved",_0: a};};
   return _elm.State.values = {_op: _op
                              ...
                              ,update: update
                              ,convertMousePosition: convertMousePosition};
};
Elm.Levels = Elm.Levels || {};
Elm.Levels.View = Elm.Levels.View || {};
Elm.Levels.View.make = function (_elm) {
   "use strict";
   _elm.Levels = _elm.Levels || {};
   _elm.Levels.View = _elm.Levels.View || {};
   if (_elm.Levels.View.values) return _elm.Levels.View.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var _op = {};
   var yellow = A3($Color.rgb,255,255,0);
   var clearGrey = A4($Color.rgba,111,111,111,0.6);
   var darkYellow = A3($Color.rgb,20,20,0);
   var red = A3($Color.rgb,255,0,0);
   var lightGradient = function (_p0) {
      var _p1 = _p0;
      var _p3 = _p1._1;
      var _p2 = _p1._0;
      ...
      {ctor: "_Tuple2",_0: _p2,_1: _p3},
      350,
      _U.list([{ctor: "_Tuple2",_0: 0,_1: A3($Color.rgb,244,242,1)},{ctor: "_Tuple2",_0: 1,_1: A4($Color.rgba,228,199,0,0)}]));
   };
   var detector = function (_p4) {
      var _p5 = _p4;
      var _p6 = _p5._0;
      var detectorColor = _U.eq(_p6.litGoal,0) ? $Color.black : $Color.yellow;
      var detector = A2($Graphics$Collage.move,_p6.position,A2($Graphics$Collage.filled,detectorColor,$Graphics$Collage.circle(15)));
      var satisfiedColor = _U.eq(_p5._1,_p6.litGoal) ? $Color.green : $Color.red;
      var satisfied = A2($Graphics$Collage.move,_p6.position,A2($Graphics$Collage.filled,satisfiedColor,$Graphics$Collage.circle(5)));
      return _U.list([detector,satisfied]);
   };
   var scene = function (state) {
      var _p7 = state.gameState;
      var level = _p7._0;
      var calculated = _p7._1;
      var triangles = A2($List.map,function (_p8) {    return A2($Graphics$Collage.filled,darkYellow,$Graphics$Collage.polygon(_p8));},level.shapes);
      var lightPosition = level.lightPosition;
      var lightMaps = A2($List.map,
      function (_p9) {
         return A2($Graphics$Collage.gradient,lightGradient(lightPosition),$Graphics$Collage.polygon(_p9));
      },
      calculated.lightMaps);
      var backdrop = A2($Graphics$Collage.filled,darkYellow,$Graphics$Collage.polygon(level.border));
      var detectors = $List.concat(A2($List.map,detector,calculated.detectors));
      var _p10 = state.windowDimensions;
      var windowWidth = _p10._0;
      var windowHeight = _p10._1;
      var _p11 = state.mousePosition;
      var mouseX = _p11._0;
      var mouseY = _p11._1;
      return A3($Graphics$Collage.collage,
      windowWidth,
      windowHeight,
      A2($Basics._op["++"],_U.list([backdrop]),A2($Basics._op["++"],triangles,A2($Basics._op["++"],lightMaps,detectors))));
   };
   return _elm.Levels.View.values = {_op: _op,scene: scene};
};
Elm.View = Elm.View || {};
Elm.View.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Levels$View = Elm.Levels.View.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var scene = F3(function (actionAddress,taskAddress,state) {    return $Levels$View.scene(state);});
   return _elm.View.values = {_op: _op,scene: scene};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $CycleApp = Elm.CycleApp.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   var _op = {};
   var windowSizes = A2($Signal.map,$State.WindowSized,$Window.dimensions);
   var mouseMoves = A2($Signal.map,$State.MouseMoved,$Mouse.position);
   var appConfig = {initialState: $State.initialState,update: $State.update,view: $View.scene};
   var app = $CycleApp.create(appConfig);
   var main = app.output;
   var actionDispatcher = $Signal.map(function (action) {    return A2($Signal.send,app.notificationAddress,action);});
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",
   $Signal.mergeMany(_U.list([actionDispatcher(windowSizes),actionDispatcher(mouseMoves),app.tasksToRun])));
